  ...
  const handleSubmit = (e) => {
    ...
    if (isLogin) {
      ...
      if (stage === 2) {
        ...
        if (exists) {
          showToast("User already exists with this username, phone, or email.", "error");
          return;
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-green-100 flex items-center justify-center px-4 py-12">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md relative overflow-hidden">
        ...
        <div className="absolute -bottom-8 -left-8 w-32 h-32 bg-green-100 rounded-full opacity-50 animate-pulse" style={{ animationDuration: "6s" }}></div>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          >
            <div className="relative flex">
              <div
                className={`absolute h-full rounded-full bg-green-600 transition-all duration-300 ${isLogin ? "w-1/2 translate-x-full" : "w-1/2 translate-x-0"}`}
              ></div>
              <div className={`py-2 px-4 rounded-full cursor-pointer text-sm font-medium z-10 transition-colors duration-300 min-w-16 text-center ${!isLogin ? "text-white" : "text-green-700"}`}>
                Sign Up
              </div>
              <div className={`py-2 px-4 rounded-full cursor-pointer text-sm font-medium z-10 transition-colors duration-300 min-w-16 text-center ${isLogin ? "text-white" : "text-green-700"}`}>
                Log In
        ...
        <p className="text-green-600 mb-6 text-sm">
          {isLogin ? "Sign in to continue your journey" : "Join Kwuo today and get started"}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {!isLogin && codeSent && stage === 1 && (
            <div className="space-y-1">
              <label className="block text-sm font-medium text-green-700">Verification Code</label>
              <div className="flex gap-2">
          ...
          {isLogin && (
            <>
              <div className="space-y-1">
                <label className="block text-sm font-medium text-green-700">Email or Phone</label>
                <input
                  type="text"
                  name="email"
                  required
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <label className="block text-sm font-medium text-green-700">PIN</label>
          ...
          >
            <span className="font-medium">
              {isLogin ? "Log In" : stage === 2 ? "Create Account" : "Verify Code"}
            </span>
          </button>
        </form>
